...
impl AccessToken {
    ...
    pub async fn new(driver: &Database, client: &OAuth2Client, espo_user_id: String) -> Result<Self> {
    ...
    pub async fn get_if_valid_for(driver: &Database, token: &str, client: &OAuth2Client) -> Result<Option<AccessToken>> {
        Ok(sqlx::query_as("SELECT * FROM oauth2_access_tokens WHERE token = ? AND client_id = ?")
            .bind(token)
            .bind(&client.client_id)
