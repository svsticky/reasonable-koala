...
impl<T: Responder<Body = BoxBody>> Responder for OAuth2AuthorizationResponse<T> where
...
impl OAuth2Error {
    pub fn new<S1: AsRef<str>, S2: AsRef<str>>(kind: OAuth2ErrorKind, redirect_uri: S1, state: Option<S2>) -> Self {
        OAuth2Error {
            kind,
