...
pub async fn remove(database: WDatabase, auth: Auth, payload: web::Json<Request>) -> WebResult<Empty> {
    if !auth.has_scope(MANAGE_SCOPE) {
        return Err(WebError::Forbidden);
    }

    let client = OAuth2Client::get_by_client_id(&database, &payload.client_id).await?
